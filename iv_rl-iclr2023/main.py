...
parser.add_argument("--max_steps", type=int, default=210,
...
if __name__ == "__main__":
    ...
    if "sac" not in opt.model.lower():
        ...
        env.seed(opt.env_seed) # When using gym
        # Do this later on env.reset(seed=seed) when using gymnasium
        
        # print(env.reset(seed=opt.env_seed)) # When using gymnasium

        # env = gym.make("CartPole-v1")
        # env.seed(opt.env_seed)
        agent = Model(env, opt, device=device)
        scores = agent.train(n_episodes=opt.num_episodes, eps_decay=opt.eps_decay, max_t=opt.max_steps, seed=opt.env_seed)
        print("scores : ", scores, "\n")
        plt.plot(scores)
        plt.xlabel("Episodes")
