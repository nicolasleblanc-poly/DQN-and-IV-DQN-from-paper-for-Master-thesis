...
from .ensembleSAC import VarEnsembleSAC 

class SunriseSAC(TorchTrainer):
    def __init__(
            self,
            args,
            env,
    ...
    def get_weights(self, std, feedback_type):
        if feedback_type == 0 or feedback_type == 1:
            weight_target_Q = torch.sigmoid(-std*self.temperature) + 0.5
        else:
            weight_target_Q = 2*torch.sigmoid(-std*self.temperature)
    ...
    def train_from_torch(self, batch):
        ...
        for en_index in range(self.num_ensemble):
            ...
            weight_actor_Q = self.get_weights(std_Q, self.feedback_type)
            ...
            if self.feedback_type == 0 or self.feedback_type == 2:
                std_Q_critic = std_Q_critic_list[en_index]
            else:
                std_Q_critic = std_Q_critic_list[0]

            weight_target_Q = self.get_weights(std_Q_critic, self.feedback_type)
...
class Sunrise_VarEnsembleSAC(VarEnsembleSAC):
    def __init__(
            self,
            args,
            env,
