...
class BootstrapDQN(MaskEnsembleDQN):
    ...
    def train(self, n_episodes=1000, max_t=1000, eps_start=1.0, eps_end=0.01, eps_decay=0.995, seed=0):
        """Deep Q-Learning.
        ...
        for i_episode in range(1, n_episodes+1):
            state = self.env.reset() # When using gym
            # state = self.env.reset(seed=seed) # When using gymnasium
            if type(state) == tuple:
                state = state[0]
            ...
            for t in range(max_t):
                ...
                # next_state, reward, done, truncated, _ = self.env.step(action) # When using gymnasium
                next_state, reward, done, _ = self.env.step(action) # When using gym
